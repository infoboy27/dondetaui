...
import { SearsAdapter } from './sears.adapter'
import { SEARS_CONFIG } from './sears.config'
...
const maxProducts = Number(process.env.SEARS_MX_MAX_PRODUCTS ?? 1000)
const requestDelayMs = Number(process.env.SEARS_MX_REQUEST_DELAY_MS ?? 500)

const pool = new Pool({ connectionString: databaseUrl, max: 5 })
const repository = new IngestionRepository(pool)
const adapter = new SearsAdapter(SEARS_CONFIG)
  ...
  try {
    ...
    while (true) {
      ...
      try {
        ...
        console.warn(`[sears-mx] page fetch failed for page ${page}: ${error instanceof Error ? error.message : String(error)}`)
        break
      }
      ...
      for (const hit of result.hits) {
        ...
        try {
          ...
          ingested += 1
          console.log(`[sears-mx] ${ingested} ${item.externalSku} ${item.name} ${formatPrice(item.price)}`)
        } catch (error) {
          skipped += 1
          console.warn(`[sears-mx] skipped ${hit.objectID}: ${error instanceof Error ? error.message : String(error)}`)
        }
      }
    ...
    console.log(`[sears-mx] completed ${ingested}/${seen}; skipped=${skipped}`)
  } catch (error) {
    await pool.query(
